...
⚠**注意**：根据题意，第一个窗口的下标范围是 $[0,n-2]$，所以窗口的大小是 $n-1$。
...
把 $s$ 视作一个环形字符串，$s$ 的某个循环移位是环形字符串在某一点断开后的结果。

设环形字符串 $s$ 有 $c$ 个相邻相同位置（注意 $s[n-1]$ 与 $s[0]$ 是相邻的）。

分类讨论：

- 如果 $c=k$，那么在相邻字母不同的位置断开，字符串的得分恰好为 $k$。这样的断点位置有 $n-c$ 个。
- 如果 $c=k+1$，那么在相邻字母相同的位置断开，字符串的得分恰好为 $k$。这样的断点位置有 $c$ 个。
- 其余情况，无法使字符串的得分恰好为 $k$。
